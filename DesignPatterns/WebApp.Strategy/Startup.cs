...
namespace WebApp.Strategy
{
    public class Startup
    {
        ...
        {
            services.AddScoped<IProductRepository>(servicesProvider =>
            {
                var httpContextAccessor = servicesProvider.GetRequiredService<IHttpContextAccessor>();
                var claim = httpContextAccessor.HttpContext.User.Claims.Where(x => x.Type == Settings.ClaimDatabaseType).FirstOrDefault();
                var context = servicesProvider.GetRequiredService<AppIdentityDbContext>();
